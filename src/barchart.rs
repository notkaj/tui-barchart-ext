        ...
        for (ticks_vec, group) in group_ticks.iter().zip(&self.data) {
            for (ticks, bar) in ticks_vec.iter().zip(&group.bars) {
                ...
                if bar_rem > 0 {
                    ...
                    for x in 0..bar_width {
                        buf[(bar_x + x, area.bottom() - bar_length - 1)]
                            .set_symbol(sym)
                            .set_style(bar_style);
                    }
                }
            ...
            for (ticks, bar) in ticks_vec.into_iter().zip(group.bars.iter()) {
                ...
                for y in 0..self.bar_width {
                    for i in 0..bar_length {
                        buf[(bars_area.right() - 1 - i, bar_y + y)]
                            .set_symbol(self.bar_set.full)
                            .set_style(bar_style);
                    }
                }
                ...
                if bar_rem > 0 {
                    ...
                    for y in 0..self.bar_width {
                        let bar_y = bar_y + y;
                        buf[(bars_area.right() - bar_length - 1, bar_y)]
                            .set_symbol(sym)
                            .set_style(bar_style);
                    }
                }
